...
with tab_players:
    ...
    if not players:
        ...
        st.markdown("""
        <div style="color:#7a9e87; font-family:'DM Mono',monospace; font-size:0.72rem; 
                    letter-spacing:1px; margin-bottom:1rem; line-height:1.8;">
        ⚠️ ON STREAMLIT CLOUD, DATA RESETS WHEN THE APP RESTARTS.<br>
        EXPORT YOUR HISTORY AFTER EACH SESSION AND IMPORT IT NEXT TIME.
        </div>
        """, unsafe_allow_html=True)
        ...
        with col_exp:
            st.markdown("**📥 Export**")
            st.caption("Download your full player history as a file.")
            json_data = export_json()
            st.download_button(
                label="⬇️  Download history.json",
                data=json_data,
                file_name="poker_history.json",
        ...
        with col_imp:
            st.markdown("**📤 Import**")
            st.caption("Upload a previously exported history file.")
            uploaded = st.file_uploader(
                "Upload poker_history.json",
                type=["json"],
